...
parboil_directory = "/root/parboil_GPU/"
polybench_directory = "/root/polybench/OpenCL/"
rodinia_directory = "/root/rodinia_GPU/opencl"
rodinia_particlefilter_directory = "/root/rodinia_GPU/opencl/particlefilter"
amd_directory = "/root/AMDAPP_samples_GPU/cl"
...
def run_parboil(benchmark_row0, benchmark, dataset):
    ...
    for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
        if row_number > 155 and row_number < 233 and row[0] and row[0].startswith(benchmark_row0):
...
def run_polybench(benchmark_row0, path, run_command):
    ...
    if match1:
        ...
        for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
            if  row_number > 155 and row_number < 233 and row[0] and row[0].startswith(benchmark_row0):
                sheet.cell(row=row_number, column=23, value=formatted_number1)
                break
        workbook.save(data_path)
    else:
    ...
    if match2:
        ...
        for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
            if  row_number > 232 and row[0] and row[0].startswith(benchmark_row0):
                sheet.cell(row=row_number, column=23, value=formatted_number2)
                break
        workbook.save(data_path)
    else:
...
def run_rodinia_nn(): # 这个看的是Exec:
    ...
    sheet = workbook.active
    for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
        if row_number > 155 and row_number < 233 and row[0] and row[0].startswith("rodinia_nn"):
            sheet.cell(row=row_number, column=23, value=formatted_number)
            break
    workbook.save(data_path)
...
def run_rodinia_pathfinder(): # 很奇怪，编译时找不到timing.h，在这里跑找不到编译好的文件
    ...
    sheet = workbook.active
    for row_number, row in enumerate(sheet.iter_rows(values_only=True), start=1):
        if row_number > 155 and row_number < 233 and row[0] and row[0].startswith("rodinia_pathfinder"):
            sheet.cell(row=row_number, column=23, value=formatted_number)
            break
    workbook.save(data_path)
